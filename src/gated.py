...
def get_pad(in_,  ksize, stride, atrous=1):
    ...
    # print("############",out_)
    # print(int(((out_ - 1) * stride + atrous*(ksize-1) + 1 - in_)/2))
...
class GatedConv2dWithActivation(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,batch_norm=True, activation=torch.nn.LeakyReLU(0.2, inplace=True)):
        ...
        self.instance_norm2d = torch.nn.InstanceNorm2d(out_channels)
...
class GatedConv2dWithActivationSP(torch.nn.Module):
    """
    Gated Convlution layer with activation (default activation:LeakyReLU)
    Params: same as conv2d
    Input: The feature from last layer "I"
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,batch_norm=True, activation=torch.nn.LeakyReLU(0.2, inplace=True)):
        super(GatedConv2dWithActivationSP, self).__init__()
        self.batch_norm = batch_norm
        self.activation = activation
        self.conv2d = nn.utils.spectral_norm(torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias))
        self.mask_conv2d = nn.utils.spectral_norm(torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias))
        self.batch_norm2d = torch.nn.BatchNorm2d(out_channels)
        self.instance_norm2d = torch.nn.InstanceNorm2d(out_channels)
        self.sigmoid = torch.nn.Sigmoid()
