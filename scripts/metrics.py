...
def parse_args():
    ...
    parser.add_argument('--data_path', help='Path to ground truth data', type=str)
    parser.add_argument('--output_path', help='Path to output data', type=str)
    parser.add_argument('--txt', help='Path to output data', type=str)
...
for fn in sorted(files):
    ...
    # img_gt = imresize(img_gt, [256,256])
    # img_pred = img_pred.resize((228, 228))
    # img_pred.imwrite("img.png")
    # img_pred = np.array(Image.fromarray(img_pred).resize(size=(228,228)))
...
with open(args.txt,mode="a") as f:
    f.write("PSNR: %.4f  " % round(np.mean(psnr), 4))
    f.write("SSIM: %.4f  " % round(np.mean(ssim), 4))
    f.write("MAE: %.4f  " % round(np.mean(mae), 4))